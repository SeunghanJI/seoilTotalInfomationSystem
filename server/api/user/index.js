...
const generateAccount = (accountInfo) => {
  return new Promise((resolve, reject) => {
    db.run(
      `insert into account(id,password) values(${accountInfo.id},"${accountInfo.password}")`,
      (err) => {
        if (err) {
...
const checkId = (id) => {
  if (100000000 < id && id < 999999999) return 'student';
  else if (1000000 < id && id < 9999999) return 'professor';
  else return '';
};
...
const getStudentInfo = (tableName, id) => {
  return new Promise((resolve, reject) => {
    db.get(
      `select id,${tableName}.name "name",birthday,dept.name "deptName", email, phone_num "phoneNum",is_break "isBreak",is_agree_collection_data "isAgreeCollectionData" 
                from ${tableName}
                inner join dept
...
const getSimpleUserInfo = (tableName, id) => {
  return new Promise((resolve, reject) => {
    db.get(
      `select id,${tableName}.name as name,dept.name as deptName from ${tableName}
                inner join dept
                on ${tableName}.department = dept.code
                where id = ${id}`,
      (err, row) => {
...
const getUserAddress = (tableName, id) => {
  return new Promise((resolve, reject) => {
    db.get(
      `select zip_code "zipCode",region,detail_address "detail" from ${tableName}
                where id = ${id}`,
      (err, row) => {
        if (err) {
          reject(ERROR_CODE[500]);
...
const insertStudent = (studentInfo) => {
  const sql = `insert into student(${Object.keys(studentInfo).join(
    ','
  )}) values(${Object.values(studentInfo).map((v) =>
    isNaN(v) ? `"${v}"` : v
  )})`;
  return new Promise((resolve, reject) => {
...
const insertProfessor = (professorInfo) => {
  const sql = `insert into professor(${Object.keys(professorInfo).join(
    ','
  )}) values(${Object.values(professorInfo).map((v) =>
    isNaN(v) ? `"${v}"` : v
  )})`;
  return new Promise((resolve, reject) => {
    db.run(sql, function (err) {
...
const updateUser = (userId, tableName, updateInfo) => {
  updateInfo = {
    ...
    ...(updateInfo.isBreak !== undefined && {
      is_break: updateInfo.isBreak ? 1 : 0,
    }),
    ...(updateInfo.isAgreeCollectionData !== undefined && {
      is_agree_collection_data: updateInfo.isAgreeCollectionData ? 1 : 0,
    }),
  };
  const table = `update ${tableName} set `;
  const update = Object.entries(updateInfo)
    .map(([key, value]) => `${key} = ${isNaN(value) ? `"${value}"` : value}`)
    .join(', ');
  const conditional = ` where id = ${userId}`;
  return new Promise((resolve, reject) => {
    db.run(table + update + conditional, function (err) {
      if (err) {
        reject(ERROR_CODE[500]);
      }
      resolve(this.lastID);
...
app.get('/', (req, res) => {
  ...
  getId(session)
    .then((user) => {
      return Promise.all([
        getStudentInfo(checkId(user.id), user.id),
  ...
  insertStudent(body)
    .then((rowid) => {
      return generateAccount({
        id: rowid,
        password: encryptString(body.birthday.split('/').join('')),
      });
    })
    .then((ignore) => {
...
app.patch('/', (req, res) => {
  ...
  getId(session)
    .then((user) => {
      return Promise.all([
        user,
        !!base && updateUser(user.id, checkId(user.id), base),
        !!address && updateAddress(user.id, 'user_address', address),
        !!currentAddress &&
          updateAddress(user.id, 'user_current_address', currentAddress),
      ]);
    })
    .then(([user, ignore]) => {
      return Promise.all([
  ...
  getId(session)
    .then((user) => {
      return updateUserPassword(
        user.id,
        encryptString(body.newPassword),
        encryptString(body.password)
      );
    })
    .then((success) => {
